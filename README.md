...
- [服务器逻辑小白版](docs/Rocket_SERVER_LOGIC.md)：可直接发给程序员，用于从零制作 Rocket Crash 游戏服务器，逐项说明回合、爆点、奖池、结算、机器人、后台和异常逻辑。
- [动态风控迭代方案](docs/Rocket_RISK_CONTROL_PLAN.md)：说明动态平台优势、即爆调整、单局亏损上限、奖池保护、风险等级和平台赚多后的放水机制。
...
- 新手引导：首次进入自动播放，使用当前游戏界面的假演练流程；完成或跳过状态记录在服务器，玩家也可以在 `Rules` 弹窗中手动重播。
...
- 玩家上次保存的 Auto Cashout 倍率记录在服务器，下次登录会自动填回输入框。
- 飞行倍率公式是分段曲线：20x 前使用慢启动幂函数，20x 后使用指数函数，显示时向下保留两位小数。
- 公式内置 2% 线性抬升保护，避免起飞后长时间停在 `1.00x`。
- 倒计时结束后玩家端先显示 1 秒 `Starting Soon!`，随后从 `1.00x` 开始飞；这样画面比服务端权威时间保守 1 秒。
- 后台 `/manage` 可以配置 `20x 到达时间 ms`、`20x 前慢启动`、`20x 后加速 ms`。
- 如果觉得 20x 前太快，优先调大 `20x 到达时间 ms`，例如 `45000`。
- 如果觉得刚起飞太快，调大 `20x 前慢启动`，例如 `2.8`。
- 如果觉得 20x 后太慢，调小 `20x 后加速 ms`，例如 `9000`。
